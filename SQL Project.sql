...
drop view vw_CreditCardDoneTransaction;
...
SELECT CONCAT(IDShop, ' ', SUBSTR(Owner, LOCATE(' ', Owner), 
LENGTH(Owner))) AS 'Owner Name' FROM trShop WHERE isOfficial = 1;
...
CONCAT(a.Name, CASE 
		WHEN isOfficial = 1 THEN ' (Official)' ELSE ' (Non-Official)' END) AS Name, Owner
FROM trShop a
...
DATE_FORMAT(TransactionDate, '%d-%M-%Y') AS "Transaction Date", qty, totalprice, paymentmethod
FROM TrTransaction
